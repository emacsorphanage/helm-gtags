...
(defun helm-gtags--current-funcion-bound (file ref-line)
  (with-temp-buffer
    (unless (zerop (process-file "global" nil t nil "-f" file))
      (error "Fail: global -f %s" file))
    (goto-char (point-min))
    (let (start-line end-line finish)
      (while (and (not finish) (not (eobp)))
        (let* ((cols (split-string (helm-current-line-contents) nil t))
               (lineno (string-to-number (cl-second cols))))
          (if (>= lineno ref-line)
              (setq finish t end-line lineno)
            (setq start-line lineno))
          (forward-line 1)))
      (cons start-line end-line))))

(defun helm-gtags--tagnames-refered-from-this-function ()
  (let* ((file (helm-gtags--real-file-name))
         (bound (helm-gtags--current-funcion-bound file (line-number-at-pos)))
         (start-line (car bound))
         (end-line (cdr bound)))
    (with-temp-buffer
      (unless (process-file "global" nil t nil "-f" "-r" file)
        (error "Failed: global -r -f %s" file))
      (goto-char (point-min))
      (let (funcs finish)
        (while (and (not finish) (not (eobp)))
          (let* ((cols (split-string (helm-current-line-contents) nil t))
            ...
            (if (and (> lineno start-line) (< lineno end-line))
                (let* ((func (cl-first cols))
                       (elm (cl-find func funcs :test 'equal)))
                  (unless elm
                    (push func funcs)))
...
;;;###autoload
(defun helm-gtags-call-from-here ()
  (interactive)
